...
// FFieldClass / FField / FProperty / FProperty-subclasses are synthesized by
// Dumper::SynthesizeReflectionTypes() into CoreUObject_structs.hpp using probed
// UE_Offsets — preamble used to hard-code UE 5.x default layout here, which
// silently desynced from the runtime when a per-game profile shifted the
// layout (DeltaForce / etc). See docs/architecture/reflection-emit.md.
